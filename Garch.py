...
class GARCH():
    ...
    def compute_variance(
        ...
        sigma2 = garch_recursion(
            parameters, bas, sigma2, p, o, q, nobs, backcast, var_bounds
        )
    ...
    def starting_values(self, resids: Float64Array) -> Float64Array:
        ...
        target = np.mean(abs(resids))
        scale = np.mean(resids) / (target)
    ...
    def _gaussian_loglikelihood(
        ...
        return float(self._normal.loglikelihood([2], resids, sigma2, 0))